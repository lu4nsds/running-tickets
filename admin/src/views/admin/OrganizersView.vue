<template>
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-white">Organizadores</h1>
        ...
        <!-- Empty State -->
        <div
            v-else-if="store.isEmpty && !store.isLoading"
        ...
        <!-- Table -->
        <div
            v-if="!store.isLoading && store.organizers.length > 0"
            class="bg-card-bg rounded-xl border border-surface-elevated overflow-hidden"
        >
            <table class="w-full">
                <thead>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button
                                    ...
                                    class="p-2 text-text-muted hover:text-primary hover:bg-surface-elevated rounded-lg transition-colors"
                                    title="Editar"
                                >
                                    <span
                                        class="material-symbols-outlined text-[20px]"
                                ...
                                <button
                                    ...
                                    class="p-2 text-text-muted hover:text-red-400 hover:bg-surface-elevated rounded-lg transition-colors"
                                    title="Excluir"
                                >
                                    <span
                                        class="material-symbols-outlined text-[20px]"
            ...
            >
                <p class="text-text-muted text-sm">
                    ...
                    {{ store.pagination.total }} resultados
                </p>
                <div class="flex items-center gap-1">
                    <!-- Prev -->
                    <button
...
const paginationInfo = computed(() => {
    ...
    const perPage = store.pagination.perPage;
...
const viewOrganizer = (organizer) => {
    ...
    toast.info(`Ver detalhes de ${organizer.name}`);
};

const editOrganizer = (organizer) => {
    // TODO: Abrir modal de edição
    toast.info(`Editar ${organizer.name}`);
};

const confirmDelete = async (organizer) => {
    if (confirm(`Tem certeza que deseja excluir "${organizer.name}"?`)) {
        const result = await store.deleteOrganizer(organizer.id);
        if (result.success) {
